...
export default function ContactModal({
  ...
  const submit = async (e: FormEvent) => {
    ...
    type StoredRequest = FormState & { id: number; createdAt: string };
    const rows = readJson<StoredRequest[]>(STORAGE_KEYS.contactRequests, []);
    const id = rows.length + 1;
    try {
      writeJson(STORAGE_KEYS.contactRequests, [
        ...rows,
        {
          ...form,
